...
class MultiEOSPoolHead(nn.Module):
    def __init__(self):
        super().__init__()
...
class VisualRWKVEmbed(pl.LightningModule):
    ...
    def preparing_embedding_text_only(self, samples):
        ...
        return query_ids_embeds, query_mask, positive_ids_embeds, positive_mask
    
    def forward(self, samples):
        if "images" in samples:
            query_emb, query_mask, positive_emb, positive_mask = self.preparing_embedding_fused(samples)
        else:
            query_emb, query_mask, positive_emb, positive_mask = self.preparing_embedding_text_only(samples)

        B, L, D = query_emb.shape
        # get the features from rwkv
        concat_input = torch.cat((query_emb, positive_emb), dim=0)
        res = self.rwkv(concat_input)
        query_multi_feature = res[:B, :, :][query_mask].view(B, -1, D)
        query_feature = self.head(query_multi_feature, samples['query_eos_mask'])  # -> [B,D]
        positive_multi_feature = res[B:, :, :][positive_mask].view(B, -1, D)
        positive_feature = self.head(positive_multi_feature, samples['positive_eos_mask'])

        return query_feature, positive_feature, query_multi_feature, positive_multi_feature
    ...
    def _contrastive_loss(
        ...
        pos_vec,
        *,
        gathered_query_vec=None,
        gathered_pos_vec=None,
        world_size=1,
    ):
        if world_size > 1:
            logits_i2t = query_vec @ gathered_pos_vec.t()
            logits_t2i = pos_vec @ gathered_query_vec.t()

            rank = getattr(self, "global_rank", 0)
            batch_size = query_vec.size(0)
            targets = torch.arange(batch_size, device=query_vec.device) + rank * batch_size
        else:
            logits_in = query_vec @ pos_vec.t()  # [B, B]
            logits_i2t = logits_in
            logits_t2i = logits_in.t()
            targets = torch.arange(logits_in.size(0), device=logits_in.device)

        logits_i2t = logits_i2t / self.args.temperature
        logits_t2i = logits_t2i / self.args.temperature
    ...
    def training_step(self, batch):
        query_vec, pos_vec, _, _ = self(batch)   # [B, D]
        ...
        if world_size > 1:
            gathered_query_vec = self.all_gather(query_vec, sync_grads=True)
            gathered_pos_vec = self.all_gather(pos_vec, sync_grads=True)

            gathered_query_vec = gathered_query_vec.reshape(-1, query_vec.shape[-1])
            gathered_pos_vec = gathered_pos_vec.reshape(-1, pos_vec.shape[-1])
        else:
            gathered_query_vec = None
            gathered_pos_vec = None
        ...
        contrastive_loss = self._contrastive_loss(
            query_vec,
            pos_vec,
            gathered_query_vec=gathered_query_vec,
            gathered_pos_vec=gathered_pos_vec,
