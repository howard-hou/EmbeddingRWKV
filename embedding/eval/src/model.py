...
class VisualRWKVEmbed(pl.LightningModule):
    ...
    def forward(self, samples):
        return self.represent_query(samples)
...
#################################################################################################################
    def represent_query(self, samples):
        '''
        use this to represent text
        '''
        query_emb = self.rwkv.emb(samples["query_ids"])
        query_mask = samples["query_ids"] == EOS_INDEX
        B, L, D = query_emb.shape
        query_out = self.rwkv(query_emb)
        query_multi_vec = query_out[query_mask].view(B, -1, D)
        query_feature = self.eos_pool(query_multi_vec, samples['query_eos_mask'])
        query_feature = self.head(query_feature, samples['task'])
        return query_feature, query_multi_vec
