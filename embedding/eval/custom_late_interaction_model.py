...
class CustomLateInteractionModel:
        ...
        **kwargs,
        ...
        for i in tqdm(range(0, len(sentences), batch_size), desc=task_name):
            ...
            with torch.inference_mode():
                _, multi_vec = self.model(batch)
                B, L, D = multi_vec.size()
                desired_L = self.cfg.ctx_len // self.cfg.eos_chunk_size
                if L < desired_L:
                    raise ValueError(f"Unexpected: L ({L}) < desired_L ({desired_L})")
                elif L > desired_L:
                    if L % desired_L != 0:
                        # pad to multiple of desired_L
                        pad_len = desired_L - (L % desired_L)
                        pad_vec = torch.zeros(B, pad_len, D, device=multi_vec.device, dtype=multi_vec.dtype)
                        multi_vec = torch.cat([pad_vec, multi_vec], dim=1)
                        L = multi_vec.size(1)
                    multi_vec = multi_vec.reshape(B, desired_L, L // desired_L, D).mean(dim=2)
            embeddings.append(multi_vec.to(torch.float32).cpu())
    ...
    def similarity(self, a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
        ...
        scores = torch.einsum("ash,bth->abst", a_tensor, b_tensor)
        return scores.max(dim=-1).values.sum(dim=-1).cpu()
