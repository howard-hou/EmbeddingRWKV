...
if __name__ == "__main__":
    ...
    parser.add_argument("--load_model", default="", type=str, help="path of rwkv model without the .pth suffix")
    ...
    parser.add_argument("--lr_layer_decay", default=1.0, type=float)  # from top to bottom(eg. 1.25)
    parser.add_argument("--lr_head_scale", default=5.0, type=float)  # 5.0 seems good
    ...
    parser.add_argument("--freeze_emb", default=0, type=int)  # layers to freeze
    parser.add_argument("--num_token_per_image", type=int, default=256)
    parser.add_argument("--num_negatives",type=str,default="5:3:2",help="Colon-separated ratio of easy:medium:hard negatives (e.g. 5:3:2).")
    parser.add_argument(
        "--reranker_layer_idx",
        type=str,
        default="0",
    ...
    parser.add_argument("--use_shared_state",action="store_true", help="Use the last RWKV state for every reranker layer (matches RWKV depth).")
    parser.add_argument("--eos_chunk_size", default=512, type=int, help="Chunk size for EOS token insertion.")
    parser.add_argument("--print_param_shape", default=0, type=int)  # print param shape

    parser = Trainer.add_argparse_args(parser)
    args = parser.parse_args()
    from src.utils import resolve_reranker_layer_indices, parse_reranker_layer_idx
    reranker_layer_idx = parse_reranker_layer_idx(args.reranker_layer_idx)
    args.reranker_layer_idx = resolve_reranker_layer_indices(reranker_layer_idx, args.n_layer)
    ########################################################################################################
...
"""
    ...
    from src.dataset import TextOnlyEmbedDatasetShard
    from tokenizer.rwkv_tokenizer import TRIE_TOKENIZER
    from src.model import RWKVReRanker
    from src.utils import load_reranker_blocks, parse_reranker_layer_idx
    ...
    if args.model_path:
        ...
        remapped_state = load_reranker_blocks(raw_model, args.reranker_layer_idx)
        msg = model.reranker.load_state_dict(remapped_state, strict=False)
        rank_zero_info(f"loading reranker model from {args.model_path}: {msg}")
    if args.freeze_rwkv > 0:
        model.freeze_rwkv(args.freeze_rwkv)
    if args.freeze_proj > 0 and args.vision_tower_path:
        model.freeze_proj()
    if args.freeze_emb > 0:
        model.freeze_emb(args.freeze_emb)
    ...
    ###########################################
    train_data = TextOnlyEmbedDatasetShard(args)
